...
| **Insight-D** | 배포 + 데모/E2E QA | 데모 데이터·결과 문구 검증, Docker→ECR→App Runner, 실서비스 E2E 테스트 |
| **Insight-P** | Red Team + 발표 | 공격자 시나리오, Devil's Advocate 프롬프트·공격적 테스트, 발표 준비·리허설 주도 |

원칙:
- 파일 소유권은 1인 1모듈. 남의 파일을 고치지 말고 소유자에게 요청.
- Insight 2명은 "코딩 안 하는 사람"이 아니라 **검증 계층**이다.
  Insight-D는 배포본과 데모 흐름을 끝까지 검증하고, Insight-P는 공격자 관점에서 시스템과 논리를 검증한다.
...
│   ├── llm.py             # GPT 구조화 + Devil's Adv.  [Build-B, 프롬프트는 Insight-P]
│   └── tests/
...
├── demo_data.json         # 사전 입력 데모 시나리오      [Insight-D]
...
3. 데모 시나리오 합의: 옵션 3개·기준 3개·팀원 4명 입력값의 초안 (Insight-D 주도).
   **갈등이 선명해야 함** — 표는 갈리고, 한 기준의 agreement가 확실히 LOW이고,
   flip point가 한 자릿수 %로 나오도록 Insight-D + Build-F가 숫자를 역설계.
...
| 0:30–1:30 | 화면 1 (에이전트 발주) + stats.py 골격·점수 | rooms/submit API + models | 데모 데이터 수치 확정·결과 문구 초안 + Dockerfile 작성·ECR 준비 | 공격자 시나리오·발표 시나리오 설계 |
| 1:30–2:00 | 화면 2 + stats.py: agreement | llm.py: GPT 구조화 | **hello-world 컨테이너 배포 검증** ⭐ | 발표 스켈레톤 (CONTEXT §4 기반) |
| 2:00–3:00 | 화면 3 (목데이터) + stats.py: Stability·Flip Point + 테스트 | /analysis 조립 + Devil's Advocate 연동 | 화면 1·3 문구 검수 (목데이터 화면 보며) | Devil's Advocate 프롬프트 → Build-B에 전달 (적대적 검증자 페르소나·반박 공방전·상호 피드백 방식 포함) |
| 3:00–4:00 | ── **통합** (Build-B 주도, Build-F 지원) ── || demo_data.json을 실서비스에 입력 = E2E 테스트 | API 공격 테스트 (빈 입력·동점·1명 제출·프롬프트 인젝션) |
...
- **QA 에이전트** (Insight-P):
  "배포된 API에 대해 비정상 입력 및 프롬프트 인젝션 시나리오 스크립트 작성:
  제출 0건 분석 요청, 동점, 1명 제출, 단일 기준, 특수문자·인젝션 이유 텍스트.
  각 케이스의 기대 동작을 표로 정리."
...
| 숫자 오류 | test_stats.py 손계산 대조 + 리허설에서 Insight-D가 결과 화면 수치를 데모 데이터와 교차 확인 |
