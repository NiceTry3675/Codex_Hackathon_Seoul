...
from backend.slack_identity import sign_metadata
from backend.slack_accounts import _cached_accounts
...
def command(client, text="", *, timestamp=None, secret="test-secret", **overrides):
    data = {
        "team_id": "TTEST", "channel_id": "CTEST", "user_id": "UPRIVATE", "command": "/synq",
...
def modal_payload(view_id="VTEST", **values):
    ...
    return {"type": "view_submission", "team": {"id": "TTEST"}, "user": {"id": "UPRIVATE"}, "view": {
        ...
        "private_metadata": sign_metadata("test-secret", "TTEST", "UPRIVATE", "CTEST"),
...
def test_analysis_work_starts_after_http_ack(setup):
    ...
    body = urlencode({"team_id": "TTEST", "channel_id": "CTEST", "user_id": "UPRIVATE", "command": "/synq",
