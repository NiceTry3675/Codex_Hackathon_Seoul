    ...
    return (
      <div className="mt-6 space-y-4">
        <div className="grid gap-3 sm:grid-cols-3">
          <div className="rounded-2xl bg-stone-50 p-4"><span className="text-xs text-stone-500">처음 가장 많이 선택한 항목</span><strong className="mt-1 block">{record.initial_majority_choice}</strong></div>
          <div className="rounded-2xl bg-stone-50 p-4"><span className="text-xs text-stone-500">분석 당시 평가 1위</span><strong className="mt-1 block">{record.analysis_winner}</strong></div>
          <div className="rounded-2xl bg-moss-50 p-4"><span className="text-xs text-moss-700">가장 자주 1위가 된 선택</span><strong className="mt-1 block text-moss-800">{record.robust_choice}</strong></div>
        </div>
        <div className="rounded-3xl bg-ink p-6 text-white">
          <span className="text-xs font-bold uppercase tracking-widest text-moss-100">최종 결정</span>
          ...
          <p className="mt-4 text-xs text-white/50">{new Date(record.decided_at).toLocaleString("ko-KR")} · {record.changed_from_initial ? "처음 가장 많이 선택한 항목과 다릅니다." : "처음 가장 많이 선택한 항목과 같습니다."}</p>
        </div>
      </div>
    );
  }

  return <p className="mt-6 rounded-2xl bg-stone-50 p-4 text-sm leading-6 text-stone-600">다시 계산한 결과를 확인한 뒤, 위의 <strong>결정과 근거 저장하기</strong> 버튼으로 최종 결정을 남길 수 있습니다.</p>;
}
...
function ResultsPage({ analysis, room, onRefreshAnalysis }: ResultsPageProps) {
  ...
  const stabilityAssessment = robustStability >= 0.8
    ? { label: "1위 비율 80% 이상", description: "계산한 조건 중 80% 이상에서 이 선택이 1위였습니다.", style: "bg-moss-100 text-moss-800" }
    : robustStability >= 0.6
      ? { label: "1위 비율 60~80% 미만", description: "계산한 조건 중 60% 이상, 80% 미만에서 이 선택이 1위였습니다.", style: "bg-amber-100 text-amber-900" }
      : { label: "1위 비율 60% 미만", description: "계산한 조건 중 60% 미만에서 이 선택이 1위였습니다.", style: "bg-red-100 text-red-800" };
    ...
    <div className="pb-10">
      <section className="border-b border-black/5 bg-sand">
        <div className="mx-auto max-w-5xl px-4 py-14 text-center sm:px-6 sm:py-20">
          <p className="eyebrow">03 · 평가 결과</p>
          <h1 className="mt-3 text-4xl font-semibold tracking-[-0.055em] sm:text-6xl">팀 평가 결과</h1>
          <p className="mx-auto mt-5 max-w-2xl text-lg leading-8 text-stone-600">
            현재 평가 1위는 <strong className="text-ink">{analysis.current_winner}</strong>입니다. 평가 차이와 결과가 바뀌는 조건을 확인하세요.
          </p>
          <div className="mt-8 flex flex-wrap justify-center gap-2 text-xs font-semibold text-stone-500">
            <a href="#votes" className="rounded-full bg-white px-3 py-2 hover:text-ink">선호 선택</a>
            <a href="#stability" className="rounded-full bg-white px-3 py-2 hover:text-ink">조건별 1위 비율</a>
            <a href="#conflict" className="rounded-full bg-white px-3 py-2 hover:text-ink">평가가 갈린 기준</a>
            <a href="#flip" className="rounded-full bg-white px-3 py-2 hover:text-ink">결과가 바뀌는 조건</a>
            <a href="#debate" className="rounded-full bg-white px-3 py-2 hover:text-ink">결정 전 확인 질문</a>
            <a href="#discussion" className="rounded-full bg-white px-3 py-2 hover:text-ink">함께 논의할 내용</a>
          </div>
        </div>
      </section>
      ...
      <div className="mx-auto max-w-5xl space-y-7 px-4 py-10 sm:px-6 sm:py-14">
        <section id="votes" className="card scroll-mt-28">
          <p className="eyebrow">01 · 처음 선호한 선택</p>
          <h2 className="section-title">선호 선택 비율</h2>
          ...
          <p className="mt-6 text-sm text-stone-500">가장 많은 사람이 선호한 선택과 중요도를 반영한 평가 1위는 다를 수 있습니다.</p>
        </section>

        <section id="stability" className="card scroll-mt-28">
          <p className="eyebrow">02 · 조건별 1위 비율</p>
          <h2 className="section-title">조건을 바꿔 계산한 결과</h2>
          <div className="mt-7 grid overflow-hidden rounded-3xl border border-moss-500/40 bg-moss-50 md:grid-cols-[1.15fr_0.85fr]">
            <div className="p-6 sm:p-8">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="text-xs text-stone-500">가장 자주 1위가 된 선택</span>
              </div>
              ...
              <div className="mt-6 flex items-end gap-3">
                ...
                <span className="pb-2 text-sm font-semibold text-stone-500">조건별 1위 비율</span>
              </div>
              <p className="mt-5 max-w-xl text-sm leading-6 text-stone-600">
                판단 기준의 중요도를 바꾼 1,000가지 조건 중 약 {robustPercentage * 10}가지에서 이 선택이 1위였습니다. 정답일 확률이 아니라, 기준의 중요도를 바꿔 계산한 결과입니다.
              </p>
          ...
          <details className="mt-6 rounded-2xl bg-stone-50 px-4 py-3 text-xs leading-5 text-stone-500">
            ...
            <p className="mt-2">판단 기준별 중요도의 팀 평균을 바탕으로 중요도를 바꿔 가며 순위를 1,000번 계산했습니다.</p>
            <p className="mt-1">현재 평가 1위와 조건을 바꿨을 때 가장 자주 1위가 된 선택은 다를 수 있습니다.</p>
          </details>
        </section>

        <section id="conflict" className="card scroll-mt-28 border-l-4 border-l-coral">
          <p className="eyebrow text-coral">03 · 평가 차이</p>
          <h2 className="section-title">평가가 갈린 기준</h2>
          <div className="mt-7 space-y-3">
            ...
            )) : <p className="text-sm text-stone-500">별도로 표시할 항목이 없습니다. 아래 기준별 평가 차이도 확인하세요.</p>}
          </div>
          <p className="mt-6 text-xs font-bold text-stone-500">{analysis.current_winner}에 대한 기준별 평가 일치 정도</p>
        ...
        <section id="flip" className="card scroll-mt-28">
          <p className="eyebrow">04 · 조건 변화</p>
          <h2 className="section-title">결과가 바뀌는 조건</h2>
          {nearbyFlipPoints.length === 0 && (
            <p className="mt-5 rounded-2xl bg-moss-50 p-4 text-sm font-semibold text-moss-800">검토한 범위에서 중요도를 15%p 이내로 바꾸거나 응답 하나를 제외했을 때 1위가 바뀌는 조건은 없었습니다.</p>
          )}
            ...
            {nearbyFlipPoints.map((flipPoint, index) => (
              <div key={index} className="rounded-3xl bg-ink p-6 text-white">
                {flipPoint.type === "weight" ? (
                  ...
                  <>
                    <span className="text-xs font-bold uppercase tracking-[0.18em] text-moss-100">응답 하나를 제외한 결과</span>
                    <strong className="mt-5 block text-4xl tracking-tight">응답 1개</strong>
          ...
          {theoreticalFlipPoints.length > 0 && (
            <details className="mt-5 rounded-2xl border border-black/10 bg-stone-50 p-4 text-sm text-stone-600">
              <summary className="cursor-pointer font-semibold">중요도를 크게 바꾼 조건 {theoreticalFlipPoints.length}개 보기</summary>
        ...
        <section className="overflow-hidden rounded-3xl bg-moss-700 p-6 text-white shadow-card sm:p-9">
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-moss-100">05 · 조건별 결과 요약</p>
          <div className="mt-4 grid items-end gap-7 md:grid-cols-[1fr_auto]">
            <div>
              <h2 className="text-3xl font-semibold tracking-[-0.04em] sm:text-4xl">가장 자주 1위가 된 선택</h2>
              <p className="mt-4 max-w-2xl leading-7 text-white/75">판단 기준의 중요도를 바꿔 계산했을 때 가장 자주 1위가 된 선택입니다. 현재 평가 1위와 함께 비교해 보세요.</p>
            </div>
            <div className="rounded-2xl bg-white px-5 py-4 text-right text-moss-700">
              <span className="block text-xs font-bold uppercase tracking-widest">가장 자주 1위</span>
        ...
        <section className="card">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <p className="eyebrow">중요도 조정</p>
              <h2 className="section-title">중요도 바꿔보기</h2>
          ...
          {!analysis.mean_scores ? (
            <div className="mt-7 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-900">
              평균 평가 점수가 없어 중요도를 바꾼 결과를 표시할 수 없습니다.
            </div>
          ) : (
            <div className="mt-8 grid gap-8 lg:grid-cols-[1fr_1fr]">
              ...
              <div className="rounded-3xl bg-stone-50 p-5">
                <div className="flex items-center justify-between gap-3">
                  ...
                  {winnerChanged && <span className="animate-pulse rounded-full bg-coral px-3 py-1 text-[11px] font-bold text-white">현재 평가와 1위가 달라요</span>}
                </div>
        ...
        <section id="debate" className="card scroll-mt-28">
          <p className="eyebrow">06 · 결정 전 확인</p>
          <h2 className="section-title">결정 전 확인 질문</h2>
        ...
        <section id="discussion" className="card scroll-mt-28">
          <p className="eyebrow">07 · 팀 논의</p>
          <h2 className="section-title">함께 논의할 내용</h2>
          <p className="mt-3 max-w-2xl text-sm leading-6 text-stone-500">결과가 바뀌는 조건과 평가 차이, 추가 확인이 필요한 내용을 모았습니다.</p>
        ...
        <section className="card">
          <p className="eyebrow">08 · 다시 계산</p>
          <h2 className="section-title">논의 후 다시 계산</h2>
          <p className="mt-3 max-w-2xl text-sm leading-6 text-stone-500">논의한 중요도를 입력하면 조건별 1위 비율과 결과가 바뀌는 조건을 다시 계산합니다.</p>
          <RecheckPanel room={room} analysis={analysis} onDecisionRecorded={() => setDecisionRecordRevision((revision) => revision + 1)} />
        </section>

        <section className="card">
          <p className="eyebrow">09 · 결정 기록</p>
          <h2 className="section-title">최종 결정 기록</h2>
          <p className="mt-3 text-sm leading-6 text-stone-500">저장한 선택과 결정 이유를 확인할 수 있습니다.</p>
