# 결정 전 확인 질문 — 개발 프롬프트 명세

> SynQ의 질문 생성과 답변 검토를 연결하는 개발 명세. 아래 Challenger·Defender·resolved 등은 프로토콜 용어이며, 사용자 화면에서는 [제품 용어 기준](../PRODUCT_LANGUAGE.md)에 따라 질문·팀 답변·AI 검토 결과로 표시한다.

## 사용자에게 보이는 문장

질문은 팀이 확인할 수 있는 근거나 대응 방법을 짧게 묻는다. `가설을 입증할 최소 검증은?`보다 `어떤 근거를 확인했나요?`처럼 쓴다. 입력에 없는 사실이나 숫자는 덧붙이지 않는다. AI의 검토 결과를 `문제 없음`, `성공 보장`으로 표현하지 않는다.

팀의 상태 입력과 AI의 검토 결과는 별도로 표시한다. 현재 프로토콜에는 AI 결과 이후 사람이 상태를 다시 확정하는 단계가 없으며, 해당 단계는 후속 제품 과제다. 아래의 보안 경계와 API 식별자는 문구 교정으로 변경하지 않는다.
